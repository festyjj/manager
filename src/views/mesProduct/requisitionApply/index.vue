<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button  type="primary" plain   @click="handleAdd" v-hasPermi="['mesProduct:requisitionApply:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button  type="success" plain   :disabled="single" @click="handleUpdate" v-hasPermi="['mesProduct:requisitionApply:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button  type="danger" plain   :disabled="multiple" @click="handleDelete" v-hasPermi="['mesProduct:requisitionApply:remove']">删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="requisitionApplyList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200" >
        <template #default="scope">
          <el-button  text  @click="handleUpdate(scope.row)" v-hasPermi="['mesProduct:requisitionApply:edit']">修改</el-button>
          <el-button  v-if="scope.row.status=='1'" text @click="handleCheck(scope.row.id,'2')" v-hasPermi="['mesProduct:requisitionApply:edit']">检验合格</el-button>
          <el-button  v-if="scope.row.status=='1'" text  @click="handleCheck(scope.row.id,'3')" v-hasPermi="['mesProduct:requisitionApply:edit']">检验不合格</el-button>
          <el-button  v-if="scope.row.auditStatus!='1'&&scope.row.applyStatus!='1'" text  @click="handleDelete(scope.row)" v-hasPermi="['mesProduct:requisitionApply:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="1000px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px" :inline="true">
        ...
        <el-form-item label="领取日期" prop="requisitionDate">
          <el-date-picker clearable  v-model="form.requisitionDate" type="date" value-format="yyyy-MM-dd" placeholder="选择领取日期" :disabled="form.auditStatus!='1'&&form.applyStatus!='1'?false:true">
          </el-date-picker>
        </el-form-item>
